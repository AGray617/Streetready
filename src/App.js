...
<style>{`@keyframes fadeUp { from { opacity:0; transform:translateY(12px); } to { opacity:1; transform:translateY(0); } } @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} } .fade-up { animation: fadeUp 0.3s ease both; } button:hover { opacity: 0.85; } textarea:focus, input:focus { outline: none; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #0d1020; } ::-webkit-scrollbar-thumb { background: #2a3a4a; border-radius: 2px; }`}</style>
...
body: “On Bible corruption: Surah 5:47 tells Christians to ‘judge by what Allah revealed in the Gospel.’ If the Gospel was corrupted — when? The Dead Sea Scrolls and 24,000 NT manuscripts show no corruption. Ask: ‘Show me the original uncorrupted version.’ On the Trinity: We agree there is one God. The question is the nature of that one God — which Scripture reveals. On Jesus never claiming divinity: John 8:58, John 10:30, John 14:9 — He claimed it repeatedly and was nearly stoned for it every time.”
},
{
...
```
const history = [...msgs, userMsg].map(m => `${m.role === "user" ? "EVANGELIST" : m.role === "objector" ? "OBJECTOR" : "COACH"}: ${m.content}`).join("\n");
...
Conversation so far:
${history}`, messages: [{ role: "user", content: `The evangelist just said: “${input}”\n\nRespond as the objector then give coaching feedback.` }],
})
});
...
setMsgs(p => […p, { role: “coach”, content: “Connection error. Check your network and try again.” }]);
}
setLoading(false);
};
...
<button onClick={() => { setScenario(null); setMsgs([]); }} style={backBtn}>← Back</button>
